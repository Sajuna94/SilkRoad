...
export default function Profile() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (user.role === UserRole.VENDOR) {
        ...
        if (logoFile) {
          // 先上傳到 Cloudinary
          const cloudinaryResult = await cloudinaryUploadMutation.mutateAsync(
            logoFile
          );
          // 再將 URL 傳給後端儲存
  ...
  const handleRemoveLogo = () => {
    setLogoFile(null);
    setLogoPreview(
      user?.role === UserRole.VENDOR && user.logo_url ? user.logo_url : null
    );
  };
  ...
  const getAvatarSrc = () => {
    ...
    return user.role === UserRole.VENDOR && user.logo_url
      ? user.logo_url
        ...
        <div className={styles.leftPanel}>
          ...
          <h2>
            ...
            {user.role === UserRole.VENDOR && (
              <span className={styles.roleTag}>(商家)</span>
            )}
          </h2>
              ...
              <div className={styles.logoUpload}>
                ...
                <div className={styles.logoActions}>
                  <input
                    ...
                    id="logo-input"
                  />
                  <label htmlFor="logo-input" className={styles.uploadLabel}>
                    選擇圖片
                  </label>
        ...
        <div className={styles.rightPanel}>
          ...
          <div className={styles.infoBlock}>
            ...
            {user.role === UserRole.ADMIN && (
              <p className={styles.adminWarning}>管理員權限已啟用</p>
            )}
          </div>
        </div>
      </section>
